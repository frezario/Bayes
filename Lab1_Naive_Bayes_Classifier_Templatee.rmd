...
naiveBayes <- setRefClass("naiveBayes",
       ...
       fields = c("counts_tidy", "prob_tw_neutr", "prob_tw_discr"),
       methods = list(
                    ...
                    predict = function(message)
                    {
                      ...
                      for (word in entries) {
                        if (word %in% counts_tidy$splitted){
                          ...
                          start_prob_neutr = start_prob_neutr * 1/(sum(counts_tidy $n_neutr) + nrow(counts_tidy))
                        }
                      ...
                      for (word in entries) {
                        if (word %in% counts_tidy$splitted){
                          ...
                          start_prob_neutr = start_prob_neutr * 1/(sum(counts_tidy $n_discrm) + nrow(counts_tidy))
                        }
...
```
```{r}
model$predict("@user @user worst customer service in the united states! already repoed 2 the better business bureau!  @user" )
```
