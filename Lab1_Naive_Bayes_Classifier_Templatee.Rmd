...
splitted_stop_words <- readLines("stop_words.txt")
# splitted_stop_words
...
```{r}
# note how SHITTY R is! Imagine writing this code in Python ...
tidy_text <- unnest_tokens(train, 'splitted', 'tweet', token = "words") %>% filter(!splitted %in% splitted_stop_words & nchar(splitted) > 2)
...
counts_tidy <- counts_tidy[order(-counts_tidy$n_overall),]
